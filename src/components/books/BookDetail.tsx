  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        ...
        setIsFavorite(true);
        toast.success("Таңдаулыларға қосылды");
      }
    } catch {
      // The optimistic state was never flipped, so nothing to roll back — just
      // tell the user it didn't take instead of silently leaving stale UI.
      toast.error("Қате шықты. Қайталап көріңіз.");
    } finally {
      setFavLoading(false);
    }
  };
